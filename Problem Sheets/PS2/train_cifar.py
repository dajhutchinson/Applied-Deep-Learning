...
parser.add_argument("--dataset-root", default=default_dataset_dir) # directory of dataset
parser.add_argument("--log-dir", default=Path("logs"), type=Path) # directory for log files
...
class CNN(nn.Module):
    ...
    def __init__(self, height: int, width: int, channels: int, class_count: int):
        ...
        self.pool1=nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2)) # first pooling operation
    ...
    # initiliase parameters: weights (N(0,sigma^2)) & bias (0s)
    @staticmethod
...
class Trainer:
    ...
    def __init__(self, model: nn.Module,
        ...
        summary_writer: SummaryWriter, device: torch.device,):
        self.model=model.to(device) # model to train
        self.train_loader=train_loader # data loader
        self.val_loader=val_loader # data loader
        self.criterion=criterion # criterion (Loss function) (softmax cross entropy)
        self.optimizer=optimizer
        self.summary_writer=summary_writer # write log summary
        self.device=device #cpu or gpu
        self.step=0

    def train(self,epochs: int,
        val_frequency: int, print_frequency: int=20, log_frequency: int=5,
        start_epoch: int=0):
        self.model.train()
        for epoch in range(start_epoch, epochs):
            ...
            for batch, labels in self.train_loader:
                ...
                self.optimizer.zero_grad() # NOTE maybe swap with line above?
